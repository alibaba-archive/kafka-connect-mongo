...
public class MongoSourceConnectorTest {
    ...
    public void taskConfigs() throws Exception {
        ...
        for (int i = 0; i < configs.size(); i++) {
            ...
            if (i == 0) {
                assertEquals("mydb.test1,mydb.test2", config.get(MongoSourceConfig.Companion.getDATABASES_CONFIG()));
            } else {
                assertEquals("mydb.test3", config.get(MongoSourceConfig.Companion.getDATABASES_CONFIG()));
            }
        }
